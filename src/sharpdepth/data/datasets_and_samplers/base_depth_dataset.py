...
# Code adapted from:
# https://github.com/prs-eth/Marigold/blob/v0.1.4/src/dataset/base_depth_dataset.py
...
class BaseDepthDataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.donwsample_factor = donwsample_factor
    ...
    def __getitem__(self, index):
        ...
        if DatasetMode.TRAIN == self.mode:
            ...
            if self.donwsample_factor is not None:
                org_imsz        = np.array(rasters["rgb_int"].shape[-2:])
                downsample_size = org_imsz // (self.donwsample_factor * 8) 
                new_size        = (int(downsample_size[0]) * 8, int(downsample_size[1]) * 8)

                nearest_transform = Resize(
                    size=new_size, interpolation=InterpolationMode.NEAREST_EXACT, antialias=True
                )
                bilinear_transform = Resize(
                    size=new_size, interpolation=InterpolationMode.BILINEAR, antialias=True
                )

                for k, v in rasters.items():
                    if "rgb" in k:
                        rasters[k] = bilinear_transform(v)
                    else:
                        rasters[k] = nearest_transform(v)
                rasters['org_imsz'] =org_imsz
        # merge
        outputs = rasters
    ...
    def _get_valid_mask(self, depth: torch.Tensor):
        ...
        if self.clip:
            depth = torch.clip(depth, self.min_depth, self.max_depth)
    ...
    def _training_preprocess(self, rasters):
        ...
        if self.depth_transform is not None:
            ...
            if self.move_invalid_to_far_plane is not None:
        ...
        if self.resize_to_hw is not None:
            nearest_transform = Resize(
                size=self.resize_to_hw, interpolation=InterpolationMode.NEAREST_EXACT, antialias=True
            )
            bilinear_transform = Resize(
